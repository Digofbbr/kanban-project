  ...
  (
    <section className='flex flex-col gap-12'>
      ...
      <div className='grid gap-4 grid-cols-1 500px:grid-cols-2 md:grid-cols-3'>
        {columns.map((col) => (
          <div key={col.id} onDragOver={(e) => handleDragOver(e)} onDrop={() => handleDrop(col.id as Card["status"])} className={`flex flex-col gap-4 items-start bg-dark-lighter rounded-sm p-4 ${draggedCard ? 'border-2 border-dashed border-primary/50' : ''}`}>
